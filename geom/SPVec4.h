...
public:
	...
	static constexpr Vec4 fill(float v) {
		return Vec4(v, v, v, v);
	}
	...
	template <typename Functor>
	constexpr Vec4(const Vec4 &v1, const Vec4 &v2, const Functor &f) : x(f(v1.x, v2.x)), y(f(v1.y, v2.y)), z(f(v1.z, v2.z)), w(f(v1.w, v2.w)) { }

	constexpr bool isValid() const { return !std::isnan(x) && !std::isnan(y) && !std::isnan(z) && !std::isnan(w); }
	...
	void add(const float &v) {
		simd::add(&this->x, v, &this->x);
	}

	void add(const Vec4 &v) {
		simd::add(&this->x, &v.x, &this->x);
	}

	void subtract(const float &v) {
		simd::sub(&this->x, v, &this->x);
	}

	void subtract(const Vec4 &v) {
		simd::sub(&this->x, &v.x, &this->x);
	}

	void scale(const float &v) {
		simd::multiply(&this->x, v, &this->x);
	}

	void scale(const Vec4 &v) {
		simd::multiply(&this->x, &v.x, &this->x);
	}

	void unscale(const float &v) {
		simd::divide(&this->x, v, &this->x);
	}

	void unscale(const Vec4 &v) {
		simd::divide(&this->x, &v.x, &this->x);
	}
