...
bool Mat4::decompose(float *scale, float *rotation, float *translation) const {
	...
	if (trace > MATH_EPSILON) {
		...
		rotation[3] = 0.25f / s;
		rotation[0] = (yaxis.z - zaxis.y) * s;
		rotation[1] = (zaxis.x - xaxis.z) * s;
		rotation[2] = (xaxis.y - yaxis.x) * s;
	} else {
		// Note: since xaxis, yaxis, and zaxis are normalized,
		// we will never divide by zero in the code below.
		if (xaxis.x > yaxis.y && xaxis.x > zaxis.z) {
			float s = 0.5f / sqrt(1.0f + xaxis.x - yaxis.y - zaxis.z);
			rotation[3] = (yaxis.z - zaxis.y) * s;
			rotation[0] = 0.25f / s;
			rotation[1] = (yaxis.x + xaxis.y) * s;
			rotation[2] = (zaxis.x + xaxis.z) * s;
		} else if (yaxis.y > zaxis.z) {
			float s = 0.5f / sqrt(1.0f + yaxis.y - xaxis.x - zaxis.z);
			rotation[3] = (zaxis.x - xaxis.z) * s;
			rotation[0] = (yaxis.x + xaxis.y) * s;
			rotation[1] = 0.25f / s;
			rotation[2] = (zaxis.y + yaxis.z) * s;
		} else {
			float s = 0.5f / sqrt(1.0f + zaxis.z - xaxis.x - yaxis.y);
			rotation[3] = (xaxis.y - yaxis.x) * s;
			rotation[0] = (zaxis.x + xaxis.z) * s;
			rotation[1] = (zaxis.y + yaxis.z) * s;
			rotation[2] = 0.25f / s;
		}
	}
