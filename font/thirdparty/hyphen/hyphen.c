 ...
 *           (C) 2023 Stappler LLC <admin@stappler.dev>
 *
...
static HashTab *hnj_hash_new(void) {
	...
	for (i = 0; i < HASH_SIZE; i++) { hashtab->entries[i] = NULL; }
...
void hnj_hyphen_load_line(char *buf, HyphenDict *dict, HashTab *hashtab) {
	...
	if (strncmp(buf, "LEFTHYPHENMIN", 13) == 0) {
		...
		if (*buf != '\0') {
			dict->nohyphen = hnj_strdup(space);
		}
		if (dict->nohyphen) {
...
*/
	...
	if (nextlevel) {
		...
		dict[1]->clhmin =
				(dict[0]->clhmin) ? dict[0]->clhmin : ((dict[0]->lhmin) ? dict[0]->lhmin : 3);
		dict[1]->crhmin =
				(dict[0]->crhmin) ? dict[0]->crhmin : ((dict[0]->rhmin) ? dict[0]->rhmin : 3);
#ifdef VERBOSE
		HashTab *r = global[0];
		global[0] = global[1];
		global[1] = r;
...
HyphenDict *hnj_hyphen_load_data(const char *fdata, size_t flen) {
	...
	for (k = 0; k < 2; k++) {
		...
		if (k == 0) {
			for (i = 0; i < MAX_NAME; i++) { dict[k]->cset[i] = 0; }
			if (hnj_hyphen_load_data_fgets(dict[k]->cset, sizeof(dict[k]->cset), &fdata, &flen)
					!= NULL) {
				for (i = 0; i < MAX_NAME; i++) {
					if ((dict[k]->cset[i] == '\r') || (dict[k]->cset[i] == '\n')) {
						dict[k]->cset[i] = 0;
...
void hnj_hyphen_free(HyphenDict *dict) {
	...
	if (dict->nextlevel) {
		hnj_hyphen_free(dict->nextlevel);
	}

	if (dict->nohyphen) {
		hnj_free(dict->nohyphen);
	}
...
int hnj_hyphen_hyphenate(HyphenDict *dict, const char *word, int word_size, char *hyphens) {
	...
	for (i = 0; i < word_size + 5; i++) { hyphens[i] = '0'; }
...
int hnj_hyphen_lhmin(int utf8, const char *word, int word_size, char *hyphens, char ***rep,
		int **pos, int **cut, int lhmin) {
	int i = 1, j;
	...
	for (j = 0; i < lhmin && word[j] != '\0'; i++) {
		do {
			// check length of the non-standard part
			if (*rep && *pos && *cut && (*rep)[j]) {
				char *rh = strchr((*rep)[j], '=');
				if (rh
						&& (hnj_hyphen_strnlen(word, j - (*pos)[j] + 1, utf8)
								   + hnj_hyphen_strnlen((*rep)[j], (int)(rh - (*rep)[j]), utf8))
								< lhmin) {
					free((*rep)[j]);
					(*rep)[j] = NULL;
					hyphens[j] = '0';
...
int hnj_hyphen_rhmin(int utf8, const char *word, int word_size, char *hyphens, char ***rep,
		int **pos, int **cut, int rhmin) {
	int i = 0;
	int j;
	...
	for (j = word_size - 1; i < rhmin && j > 0; j--) {
		...
		if (*rep && *pos && *cut && (*rep)[j]) {
			char *rh = strchr((*rep)[j], '=');
			if (rh
					&& (hnj_hyphen_strnlen(word + j - (*pos)[j] + (*cut)[j] + 1, 100, utf8)
							   + hnj_hyphen_strnlen(rh + 1, (int)strlen(rh + 1), utf8))
							< rhmin) {
				free((*rep)[j]);
				(*rep)[j] = NULL;
				hyphens[j] = '0';
		...
		if (!utf8 || (word[j] & 0xc0) == 0xc0 || (word[j] & 0x80) != 0x80) {
			i++;
		}
	}
	return 0;
...
int hnj_hyphen_hyph_(HyphenDict *dict, const char *word, int word_size, char *hyphens, char ***rep,
		int **pos, int **cut, int clhmin, int crhmin, int lend, int rend) {
	char *prep_word;
	int i, j, k;
	int state;
	char ch;
	...
	prep_word = (char *)hnj_malloc((int)prep_word_size);
...
try_next_letter:;
	...
	if (dict->nextlevel) {
		...
		for (i = 0; i < word_size; i++) {
			if (hyphens[i] & 1 || (begin > 0 && i + 1 == word_size)) {
				if (i - begin > 0) {
					...
					hnj_hyphen_hyph_(dict, prep_word + begin + 1, i - begin + 1 + hyph, hyphens2,
							&rep2, &pos2, &cut2, clhmin, crhmin, (begin > 0 ? 0 : lend),
							(hyphens[i] & 1 ? 0 : rend));
					for (j = 0; j < i - begin; j++) {
		...
		// non-compound
		if (begin == 0) {
			hnj_hyphen_hyph_(dict->nextlevel, word, word_size, hyphens, rep, pos, cut, clhmin,
					crhmin, lend, rend);
			if (!lend) {
				hnj_hyphen_lhmin(dict->utf8, word, word_size, hyphens, rep, pos, cut, clhmin);
			}
			if (!rend) {
				hnj_hyphen_rhmin(dict->utf8, word, word_size, hyphens, rep, pos, cut, crhmin);
			}
		}
...
int hnj_hyphen_norm(const char *word, int word_size, char *hyphens, char ***rep, int **pos,
		int **cut) {
	int i, j, k;
...
void hnj_hyphen_hyphword(const char *word, int l, const char *hyphens, char *hyphword, char ***rep,
		int **pos, int **cut) {
	int hyphenslen = l + 5;
...
int hnj_hyphen_hyphenate2(HyphenDict *dict, const char *word, int word_size, char *hyphens,
		char *hyphword, char ***rep, int **pos, int **cut) {
	hnj_hyphen_hyph_(dict, word, word_size, hyphens, rep, pos, cut, dict->clhmin, dict->crhmin, 1,
			1);
	hnj_hyphen_lhmin(dict->utf8, word, word_size, hyphens, rep, pos, cut,
			(dict->lhmin > 0 ? dict->lhmin : 2));
	hnj_hyphen_rhmin(dict->utf8, word, word_size, hyphens, rep, pos, cut,
	...
	if (dict->utf8) {
		return hnj_hyphen_norm(word, word_size, hyphens, rep, pos, cut);
	}
#ifdef VERBOSE
	printf("nums: %s\n", hyphens);
#endif
...
int hnj_hyphen_hyphenate3(HyphenDict *dict, const char *word, int word_size, char *hyphens,
		char *hyphword, char ***rep, int **pos, int **cut, int lhmin, int rhmin, int clhmin,
		int crhmin) {
	...
	hnj_hyphen_hyph_(dict, word, word_size, hyphens, rep, pos, cut, clhmin, crhmin, 1, 1);
	hnj_hyphen_lhmin(dict->utf8, word, word_size, hyphens, rep, pos, cut, (lhmin > 0 ? lhmin : 2));
	hnj_hyphen_rhmin(dict->utf8, word, word_size, hyphens, rep, pos, cut, (rhmin > 0 ? rhmin : 2));
	if (hyphword) {
		hnj_hyphen_hyphword(word, word_size, hyphens, hyphword, rep, pos, cut);
	}
